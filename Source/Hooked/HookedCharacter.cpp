...
AHookedCharacter::AHookedCharacter()
{
...
void AHookedCharacter::BeginPlay()
{
	// Call the base class
	Super::BeginPlay();
...
void AHookedCharacter::SetupPlayerInputComponent(class UInputComponent *PlayerInputComponent)
{
	...
	PlayerInputComponent->BindAction("ResetVR", IE_Pressed, this, &AHookedCharacter::OnResetVR);
...
{
	TArray<AHookedProjectile *> BulletArray;
	for (TActorIterator<AHookedProjectile> ActorItr(GetWorld()); ActorItr; ++ActorItr)
	{
		BulletArray.Push(*ActorItr);
	};
	UWorld *const World = GetWorld();

	
	if (BulletArray.Num() < 1)
	{
		...
		if (ProjectileClass != NULL)
		{
			...
			if (World != NULL)
			{
				if (bUsingMotionControllers)
				{
					...
					World->SpawnActor<AHookedProjectile>(ProjectileClass, SpawnLocation, SpawnRotation);
				}
				else
				{
					...
					World->SpawnActor<AHookedProjectile>(ProjectileClass, SpawnLocation, SpawnRotation, ActorSpawnParams);
				}
			}
		}
...
void AHookedCharacter::OnResetVR()
{
	UHeadMountedDisplayFunctionLibrary::ResetOrientationAndPosition();
}

void AHookedCharacter::BeginTouch(const ETouchIndex::Type FingerIndex, const FVector Location)
{
...
void AHookedCharacter::EndTouch(const ETouchIndex::Type FingerIndex, const FVector Location)
{
...
//void AHookedCharacter::TouchUpdate(const ETouchIndex::Type FingerIndex, const FVector Location)
//{
...
void AHookedCharacter::MoveForward(float Value)
{
	if (Value != 0.0f)
	{
...
void AHookedCharacter::MoveRight(float Value)
{
	if (Value != 0.0f)
	{
		// add movement in that direction
...
void AHookedCharacter::TurnAtRate(float Rate)
{
	// calculate delta for this frame from the rate information
	AddControllerYawInput(Rate * BaseTurnRate * GetWorld()->GetDeltaSeconds());
}

void AHookedCharacter::LookUpAtRate(float Rate)
{
	// calculate delta for this frame from the rate information
	AddControllerPitchInput(Rate * BaseLookUpRate * GetWorld()->GetDeltaSeconds());
}

bool AHookedCharacter::EnableTouchscreenMovement(class UInputComponent *PlayerInputComponent)
{
	if (FPlatformMisc::SupportsTouchInput() || GetDefault<UInputSettings>()->bUseMouseForTouch)
	{
		PlayerInputComponent->BindTouch(EInputEvent::IE_Pressed, this, &AHookedCharacter::BeginTouch);
		PlayerInputComponent->BindTouch(EInputEvent::IE_Released, this, &AHookedCharacter::EndTouch);

		//Commenting this out to be more consistent with FPS BP template.
		//PlayerInputComponent->BindTouch(EInputEvent::IE_Repeat, this, &AHookedCharacter::TouchUpdate);
		return true;
	}
